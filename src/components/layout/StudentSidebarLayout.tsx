...
/* Menú lateral exclusivo para usuarios votantes.
   Similar al AdminLayout pero con opciones permitidas para estudiante. */
const StudentSidebarLayout: React.FC = () => {
  ...
  const menuItems = [
    { icon: LayoutDashboard, label: 'Inicio', path: '/student/dashboard' },
    { icon: User, label: 'Mi Perfil', path: '/student/profile' },
    { icon: Lock, label: 'Cambiar Contraseña', path: '/student/change-password' },
    { icon: Clock, label: 'Historial Personal', path: '/student/history' },
    { icon: IdCard, label: 'Carnet de Sufragio', path: '/student/carnet' },
  ];
  ...
  return (
    <div className="min-h-screen flex relative">
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-72 bg-[var(--bg-sidebar)] backdrop-blur-xl
          border-r border-[var(--border-color)] transform transition-transform
          duration-300 ease-in-out lg:relative lg:translate-x-0
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-8 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-black text-[var(--text-primary)] tracking-tight">
                UPEA <span className="text-blue-500">Vota</span>
              </h1>
            </div>
            <button
              className="lg:hidden p-2 hover:bg-white/5 rounded-lg text-[var(--text-secondary)]"
              onClick={() => setIsSidebarOpen(false)}
            >
              <X size={20} />
            </button>
          ...
          <div className="p-6 border-t border-[var(--border-color)]">
            <Link
              to="/student/profile"
              className="bg-[var(--bg-tertiary)]/50 rounded-2xl p-4 mb-4 flex items-center gap-3 border border-[var(--border-color)] hover:bg-[var(--bg-tertiary)]/80 transition-colors"
            >
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold shadow-md shrink-0 overflow-hidden">
                {user?.photo_url ? (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[var(--text-tertiary)] font-medium truncate uppercase tracking-wider">
                  {user?.reg_univ}
                </p>
              </div>
            </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        <header className="h-20 bg-[var(--bg-secondary)]/60 backdrop-blur-md border-b border-[var(--border-color)] flex items-center justify-between px-6 lg:px-10 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            <button
              className="lg:hidden p-2.5 hover:bg-white/5 rounded-xl border border-[var(--border-color)] text-[var(--text-secondary)]"
