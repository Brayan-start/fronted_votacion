...
import { 
  Calendar, Clock, ArrowRight, Vote, CheckCircle2, AlertCircle,
  GraduationCap, Users, Award, RefreshCcw
} from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[1, 2, 3, 4].map((i) => (
                <Card key={i} className="animate-pulse !p-0 overflow-hidden h-64 border-[var(--border-color)]">
                  <div className="h-2 bg-[var(--bg-tertiary)]"></div>
                  <div className="p-6 space-y-4">
                    <div className="w-12 h-12 bg-[var(--bg-tertiary)] rounded-2xl"></div>
                    <div className="h-6 bg-[var(--bg-tertiary)] rounded-lg w-3/4"></div>
                    <div className="h-4 bg-[var(--bg-tertiary)] rounded-lg w-full"></div>
                    <div className="h-10 bg-[var(--bg-tertiary)] rounded-xl w-full mt-auto"></div>
                  </div>
                </Card>
              ))}
            </div>
              ...
              {elections.filter(e => e.status === 'active').map((election) => {
                ...
                return (
                  <Card key={election.id} className="group hover:shadow-2xl transition-all duration-300 !p-0 overflow-hidden flex flex-col border-[var(--border-color)] relative">
                    ...
                    <div className={`h-2 ${hasVoted ? 'bg-emerald-500' : 'bg-gradient-to-r from-blue-600 to-indigo-600'}`}></div>
                      ...
                      <div className="mt-auto space-y-3">
                        ...
                        <Button 
                          ...
                          className={`rounded-2xl py-3 group-hover:shadow-lg transition-all ${
                            hasVoted 
                              ? '!bg-emerald-500/10 !text-emerald-400 border-2 border-emerald-500/20 hover:!bg-emerald-500/20' 
                              : ''
                          }`}
                        >
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-[var(--bg-card)] backdrop-blur-xl border-[var(--border-color)] shadow-xl">
            <div className="space-y-4">
              ...
              <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 relative overflow-hidden group">
                <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-1">Votos Emitidos</p>
                <div className="flex items-center justify-between">
                  <p className="text-lg font-black text-[var(--text-primary)]">{voteCount}</p>
                  {voteCount > 0 && (
                    <span className="text-[10px] font-black bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-md uppercase tracking-tighter border border-emerald-500/20">
                      Voto registrado
                    </span>
                  )}
                </div>
